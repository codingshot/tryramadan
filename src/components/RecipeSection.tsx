        ...
        >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {recipes.suhoor.slice(0, 3).map((recipe, index) => (
              <Link key={recipe.id} to={`/recipe/suhoor/${recipe.id}`}>
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ delay: index * 0.1 }}
                  className="card-cultural group h-full hover:border-secondary/50 transition-colors"
                >
        ...
        >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {recipes.iftar.slice(0, 3).map((recipe, index) => (
              <Link key={recipe.id} to={`/recipe/iftar/${recipe.id}`}>
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
