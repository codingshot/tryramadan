...
describe("ramadan.ts", () => {
  ...
  it("getRamadanDayNumber returns 1–30 for dates in Ramadan", () => {
    const dec24_2030 = new Date(2030, 11, 24); // first day Ramadan 2031
    expect(getRamadanDayNumber(dec24_2030)).toBe(1);
  ...
  it("isRamadanDay false for dates outside Ramadan", () => {
    const jun1_2025 = new Date(2025, 5, 1); // Ramadan 2025 is Feb 28 – Mar 29
    expect(isRamadanDay(jun1_2025)).toBe(false);
  });

  it("isRamadanDay true for Jan 1 2031 (still in Ramadan 2031)", () => {
    const jan1_2031 = new Date(2031, 0, 1); // Ramadan 2031 ends Jan 22 2031
