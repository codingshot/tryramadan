...
const DashboardSchedule = () => {
  ...
  const copyTableRowTimes = useCallback(
    (dateStr: string, pt: { imsak?: string; maghrib?: string }) => {
  ...
  const handlePrayerTimesPrint = useCallback(() => {
    ...
    const rows = Object.entries(effectiveRamadanTimesMap)
      ...
      .map(([dateStr, pt]) => {
        const d = new Date(dateStr + "T12:00:00");
        const hrs = getFastingHoursForDay(pt.imsak, pt.maghrib);
        return `<tr><td>${d.toLocaleDateString("en", { month: "short", day: "numeric", year: "numeric" })}</td><td>${pt.imsak ?? "—"}</td><td>${pt.maghrib ?? "—"}</td><td>${hrs != null ? hrs + "h" : "—"}</td></tr>`;
      })
      .join("");
  ...
  const nextSevenDays = useMemo(() => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      out.push({
        dateStr,
        pt,
        fastingHrs: pt ? getFastingHoursForDay(pt.imsak, pt.maghrib) : null,
      });
    }
    return out;
                          ...
                          .map(([dateStr, pt]) => {
                            ...
                            return (
                              <tr key={dateStr} className="border-b border-border/50">
                                ...
                                <td className="py-1.5 text-right font-mono tabular-nums">{pt.imsak ?? "—"}</td>
                  ...
                  <table className="w-full min-w-[380px] text-sm border-collapse">
                    <thead>
                      <tr className="border-b border-border">
                        ...
                        <th className="w-8" aria-label="Copy times" />
                        <th className="w-8" />
                      </tr>
                    </thead>
                    <tbody>
                ...
                {nextSevenDays.map(({ dateStr, pt, fastingHrs }) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {pt ? (
                        <>
                          <span className="text-[10px] font-mono text-secondary truncate w-full text-center" title={`Suhoor end ${pt.imsak}`}>{pt.imsak ?? "—"}</span>
