...
type SortKey = "name" | "imsak" | "maghrib" | "hours";
...
/** Fasting hours from imsak (suhoor end) to maghrib (iftar). */
function fastingHours(imsak: string | undefined, maghrib: string | undefined): number | null {
...
export default function RamadanCompare() {
    ...
    try {
      const results = await Promise.all(
        regions.map(async (r) => {
          ...
          const hrs = fastingHours(pt.imsak, pt.maghrib);
          return {
            region: r,
            imsak: pt.imsak || "—",
            maghrib: pt.maghrib || "—",
  ...
  const sortedRows = [...rows].sort((a, b) => {
    ...
    switch (sortKey) {
      case "name":
        ...
        break;
      case "imsak":
        cmp = (a.imsak || "").localeCompare(b.imsak || "");
        break;
      case "maghrib":
  ...
  const webPageJsonLd = {
    ...
    description: "Compare suhoor end (Imsak), iftar (Maghrib), and total fasting hours across cities worldwide. See how fasting duration varies by region.",
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-4xl">
        ...
        <p className="text-muted-foreground mb-6">
          See suhoor end (Imsak), iftar (Maghrib), and total fasting duration for any date. Add cities below or use the presets.
        </p>
              ...
              <thead>
                <tr className="border-b border-border bg-muted/40">
                  ...
                  <th className="p-3 font-semibold text-foreground">
                    <button
                      type="button"
                      onClick={() => toggleSort("imsak")}
                      className="inline-flex items-center gap-1 hover:text-secondary focus:outline-none focus:underline"
                    >
                      Fasting start (Imsak)
                      {sortKey === "imsak" ? sortDir === "asc" ? <ArrowUp className="w-4 h-4" /> : <ArrowDown className="w-4 h-4" /> : <ArrowUpDown className="w-4 h-4 opacity-50" />}
                    </button>
                  </th>
                  <th className="p-3 font-semibold text-foreground">
                    <button
              ...
              <tbody>
                {sortedRows.map((row) => (
                  <tr key={row.region.id} className="border-b border-border last:border-b-0 hover:bg-muted/20">
                    ...
                    <td className="p-3 text-foreground">{row.error ? "—" : row.imsak}</td>
