...
// Keys identify annual Ramadan occurrences, not necessarily their Gregorian start year.
function occurrenceKey(start: Date): number {
  for (const key of Object.keys(RAMADAN_START_BY_YEAR).map(Number)) {
    if (calendarDay(getRamadanStartForYear(key)) === calendarDay(start)) return key;
  }
  // Fallback dates can also land in the preceding Gregorian year.
  for (let key = start.getFullYear() - 1; key <= start.getFullYear() + 2; key++) {
    if (calendarDay(getRamadanStartForYear(key)) === calendarDay(start)) return key;
  }
...
export function getRamadanDayNumber(date: Date): number | null {
  ...
  const dayNum = calendarDay(d) - calendarDay(start) + 1;
...
export function getDaysUntilRamadan(): number {
  ...
  start.setHours(0, 0, 0, 0);
  if (today >= start) {
    const end = getRamadanEndForYear(occurrenceKey(start));
    if (today <= end) return 0;
  }
  return Math.max(0, calendarDay(start) - calendarDay(today));
}

/** Whether we are currently within Ramadan. */
export function isCurrentlyRamadan(): boolean {
  return isRamadanDay(new Date());
}
...
export function getEffectiveRamadanRange(overrides?: RamadanOverrideInput | null): {
  ...
  if (
    ...
    const length = getRamadanRangeLength(start, end);
    if (toStr(start) === overrides.ramadanStartOverride &&
        toStr(end) === overrides.ramadanEndOverride &&
        (length === 29 || length === 30)) {
      if (today <= end) {
        return {
          start,
...
export function getRamadanDayNumberInRange(date: Date, start: Date, end: Date): number | null {
  ...
  const s = new Date(start);
  s.setHours(0, 0, 0, 0);
  const dayNum = calendarDay(d) - calendarDay(s) + 1;
...
export function getRamadanRangeLength(start: Date, end: Date): number {
  ...
  const e = new Date(end);
  e.setHours(0, 0, 0, 0);
  return calendarDay(e) - calendarDay(s) + 1;
}
