...
const DashboardToday = () => {
  ...
  const getFastingProgress = () => {
    ...
    const imsakSec = timeStringToSecondsSinceMidnight(prayerTimes.fajr);
  ...
  const imsakStr = prayerTimes?.fajr;
  ...
  useEffect(() => {
    ...
    const imsakSec = timeStringToSecondsSinceMidnight(prayerTimes.fajr);
    const imsakTomorrowSec = timeStringToSecondsSinceMidnight(imsakTomorrow ?? prayerTimes.fajr);
    const maghribSec = timeStringToSecondsSinceMidnight(prayerTimes.maghrib);
    const tick = () => {
      const nowSec = displayTimezone
        ? getNowSecondsSinceMidnightInTimezone(displayTimezone)
        ...
        <div className="container mx-auto px-4 max-w-4xl min-w-0">
          ...
          >
            ...
            <div className="flex justify-between mt-2 text-xs text-muted-foreground gap-2">
              <span className="truncate min-w-0" title={`${suhoorLabelShort} end (eat cutoff)`}><span className="sm:inline hidden">{suhoorLabelShort} end </span>{prayerTimes?.fajr ?? '05:30'}</span>
