...
describe("buildIcalContent", () => {
  it("uses Fajr rather than precautionary Imsak as the fasting boundary", () => {
    const ics = buildIcalContent({
      prayerTimesMap: { '2025-03-01': mockPrayerTimes }, customEvents: {},
      dateRange: ['2025-03-01', '2025-03-01'], includeTaraweeh: false,
      includePrayers: true, exportMode: 'fasting',
    });
    expect(ics).toContain('T052300');
    expect(ics).not.toContain('T051300');
    expect(ics).toContain('Suhoor ends (Fajr)');
  });
