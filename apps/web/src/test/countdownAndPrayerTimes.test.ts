...
describe("today prayer times cache (local storage after initial call)", () => {
  ...
  it.each([[51.5, -0.1], [0, 36.8], [51.5, 0], [0, 0]])("usePrayerTimes uses cache at %s,%s without an API call", async (lat, lng) => {
...
describe("Ramadan prayer times cache", () => {
  ...
  it("Ramadan cache key includes location, year and calculation method", () => {
    const key = getRamadanPrayersCacheKey(51.5074, -0.1278, 2026);
    expect(key).toMatch(/^[\d.-]+_[\d.-]+_2026_\d+$/);
  });
});
