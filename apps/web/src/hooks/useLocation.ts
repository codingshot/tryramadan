...
const timezoneRequests = new Map<string, Promise<string | null>>();
const timezoneCache = new Map<string, { value: string | null; expires: number }>();

export function getTimezoneFromCoords(lat: number, lng: number): Promise<string | null> {
  if (!Number.isFinite(lat) || !Number.isFinite(lng) || Math.abs(lat) > 90 || Math.abs(lng) > 180) return Promise.resolve(null);
  const key = `${lat},${lng}`;
  const cached = timezoneCache.get(key);
  if (cached && cached.expires > Date.now()) return Promise.resolve(cached.value);
  const pending = timezoneRequests.get(key);
  if (pending) return pending;
  const request = fetchTimezone(lat, lng).then((value) => {
    // Negative caching avoids a request storm when a location service is offline.
    if (timezoneCache.size >= 100) timezoneCache.delete(timezoneCache.keys().next().value!);
    timezoneCache.set(key, { value, expires: Date.now() + (value ? 86400000 : 60000) });
    return value;
  }).finally(() => timezoneRequests.delete(key));
  timezoneRequests.set(key, request);
  return request;
}
