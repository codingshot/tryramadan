...
export function useLocalStorage<T>(key: string, initialValue: T): [T, (value: T | ((prev: T) => T)) => void] {
  ...
  // Initialize only missing keys. Mirroring every render back to storage lets
  // stale mounted consumers overwrite a newer write and creates feedback loops.
  React.useEffect(() => {
    try {
  ...
  const updateValue = React.useCallback((value: T | ((prev: T) => T)) => {
    let current = valueRef.current;
    try {
      const raw = window.localStorage.getItem(key);
      if (raw !== null) current = JSON.parse(raw) ?? current;
    } catch { /* Use the last readable value. */ }
    const next = typeof value === 'function' ? (value as (prev: T) => T)(current) : value;
    valueRef.current = next;
    setStoredValue(next);
    try {
      window.localStorage.setItem(key, JSON.stringify(next));
      window.dispatchEvent(new CustomEvent('tryramadan-storage-updated', { detail: key }));
      if (key === 'tryramadan-preferences') window.dispatchEvent(new Event('tryramadan-prefs-updated'));
    } catch (error) {
      console.error(`Error setting localStorage key "${key}":`, error);
    }
  }, [key]);
  ...
  React.useEffect(() => {
    ...
    const onSameTabStorage = (event: Event) => {
      if ((event as CustomEvent<string>).detail === key) resync();
    };
    window.addEventListener('tryramadan-storage-updated', onSameTabStorage);

    return () => {
      window.removeEventListener('storage', onStorage);
      window.removeEventListener('tryramadan-prefs-updated', onPrefsUpdated);
      window.removeEventListener('tryramadan-storage-updated', onSameTabStorage);
    };
  }, [key]);
...
export function useDisplayTimezone(): string | null {
  ...
  const lat = preferences.locationCoords?.lat;
  const lng = preferences.locationCoords?.lng;
  React.useEffect(() => {
    if (lat == null || lng == null || preferences.timezone != null) return;
    let cancelled = false;
    getTimezoneFromCoords(lat, lng).then((tz) => {
      if (!cancelled && tz) setPreferences((prev) =>
        prev.locationCoords?.lat === lat && prev.locationCoords?.lng === lng && prev.timezone == null
          ? { ...prev, timezone: tz } : prev);
    });
    return () => { cancelled = true; };
  }, [lat, lng, preferences.timezone, setPreferences]);
